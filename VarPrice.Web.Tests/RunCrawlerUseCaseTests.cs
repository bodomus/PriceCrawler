...
public sealed class RunCrawlerUseCaseTests
{
    ...
    public async Task RunVegetablesAsync_Success_TransitionsStatuses()
    {
        ...
        var extractor = new FakeExtractor(new ProductCard("1", "name", "url", 10m, null, false, true, null, null, "kyiv"));
    ...
    public async Task RunVegetablesAsync_Failure_SetsErrorInfoInIngestionOnly()
    {
        ...
        var extractor = new FakeExtractor(null);
    ...
    {
        public Task<IReadOnlyList<string>> GetProductUrlsAsync(string sitemapIndexUrl, CancellationToken ct) => throw new InvalidOperationException("boom");
    }

    private sealed class FakeExtractor(ProductCard? card) : IProductCardExtractor
    {
        public Task<ProductCard?> ExtractAsync(string url, CancellationToken ct) => Task.FromResult(card);
    }
    ...
    private sealed class FakeCrawlerRunRepository : ICrawlerRunRepository
    {
        ...
        public Task FinishAsync(long runId, RunStatus status, string? note, CancellationToken ct) { LastStatus = status; return Task.CompletedTask; }
    }
